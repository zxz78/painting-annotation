...
def read_web(pagenumber, artist_name, style):
	if style=="all":
		_url = 'https://www.wikiart.org/en/'+ artist_name +'/mode/all-paintings'+'?json=2&page=%s' % pagenumber
...
def parse_web(artist_name, style,save_path):
	...
	count=1
	f1=open(save_path+artist_name+"/%s_painting_main_list.txt" % artist_name,'w')
	f2=open(save_path+artist_name+"/%s_painting_download_list.txt" % artist_name,'w')
	while True:
...
def download_painting(artist_name,style,save_path):
	url_lists=[]
	count=0
	f=open(save_path+artist_name+"/%s_painting_download_list.txt" % artist_name)
	for l in f:
		l=l.strip()
		if not l:
			continue
		url_lists.append(l)
	f.close()
	dirname = save_path+"%s_paintings" % artist_name
...
def get_details(artist_name,style,save_path):
	...
	f=open(save_path+artist_name+"/%s_painting_main_list.txt" % artist_name)
	image_list=[]
	for l in f:
		l=l.strip()
		if not l:
	...
	f.close()
	detail_list = []
	csvfile=file(save_path+artist_name+"/detail.csv",'wb')
...
if __name__=="__main__":
	...
	style = raw_input("Input the the artist's style you what:(eg: claude-monet. If you want all the paintings of this painter:all)\n")
