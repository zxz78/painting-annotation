...
def read_web(style, pagenumber):
	_url = 'https://www.wikiart.org/en/paintings-by-style/' + style + '?json=2&page=%s' % pagenumber
	return  urllib.urlopen(_url)

def parse_web(style,save_path):
	print 'start getting the painting list of %s\n' % style
	count=1
	f1=open(save_path+"%s_painting_main_list.txt" % style,'w')
	f2=open(save_path+"%s_painting_download_list.txt" % style,'w')
	while True:
		web=read_web(style,count)
		dic=json.load(web)
...
def download_painting(style,save_path):
	url_lists=[]
	count=0
	f=open(save_path+"%s_painting_download_list.txt" % style)
	for l in f:
		l=l.strip()
		if not l:
			continue
		url_lists.append(l)
	f.close()
	dirname = save_path+"%s_paintins" % style
...
def get_details(style,save_path):
	print 'start getting the painting details of %s\n' % style
	f=open(save_path+"%s_painting_main_list.txt" % style)
	image_list=[]
	for l in f:
		l=l.strip()
		if not l:
...
if __name__=="__main__":
	style = raw_input("Input the the style you what:(eg: conceptual-art)\n")
	...
	if not os.path.exists(save_path):
		...
		os.mkdir(save_path)
	parse_web(style,save_path)
	download_painting(style,save_path)
	get_details(style,save_path)
